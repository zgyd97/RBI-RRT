...
RBI_RRT::RBI_RRT(int dim, const State& start_node, const State& goal_node,
...
vector<double> RBI_RRT::sampleUnitBall() {
...
vector<vector<double>> RBI_RRT::rotationMatrix(
...
bool RBI_RRT::isValid(State stat1,State stat2)//collision checking placeholder
{
...
double RBI_RRT::distance(const vector<double>& stat_a, const vector<double>& stat_b)
{
    double sum = 0.0;
...
State RBI_RRT::steer(vector<double> stat_from, vector<double> stat_to,double step_size)
{
...
RBI_RRT::ExtendResult RBI_RRT::extend(rrtTree* tree, kdtree* tree_kd, State stat_target,double step_size)// one step
{
...
RBI_RRT::ExtendResult RBI_RRT::connect(rrtTree* tree, kdtree* tree_kd,State stat_target,double step_size)//greedy extend
{
...
void RBI_RRT::reconstructTree(rrtTree* tree)
{
...
rrtNode* RBI_RRT::chooseParent(vector<rrtNode*> nearNodes, State newState)
{
...
void RBI_RRT::rewire(rrtNode* x_new, vector<rrtNode*> nearNodes)
{
    for(rrtNode* x_near : nearNodes)
    {
...
void RBI_RRT::plan()
{
    ...
    rrtTree rrtTree1(100000), rrtTree2(100000);
...
vector<rrtNode*> RBI_RRT::getSolutionPath()
{
...
string RBI_RRT::serialize(rrtNode* root)
{
    if (!root) return "null";
...
int main()
{
    ...
    // RBI_RRT planner(3, start, goal, lb, ub);
    RBI_RRT planner(3, {{0.0,0.0,0.0}}, {{10.0,10.0,0.0}}, {{ 0.0, 0.0, 0.0}}, {{10.0,10.0,0.0}});
    planner.plan();
